...
Hooks:PostHook(MedicDamage, "init", "MutatorQuickFix_MedicDamage_init", function(medic, ...)
	if TMP_mutator_saving:Is_This_Enable("MutatorQuickFix", MutatorQuickFix) then
...
function MedicDamage:heal_unit(...)
	...
	if result == true and TMP_mutator_saving:Is_This_Enable("MutatorQuickFix", MutatorQuickFix) then
