...
Hooks:Add("LocalizationManagerPostInit", "MutatorHydra_Re_Loc", function(loc)
	LocalizationManager:add_localized_strings({
		["menu_hydra_re_toggle"] = "Reverse",
	})
end)

Hooks:PostHook(MutatorHydra, "register_values", "MutatorHydra_register_values", function(mex, ...)
	mex:register_value("toggle_hydra_re", false, "nd")
end)
...
Hooks:PostHook(MutatorHydra, "setup_options_gui", "MutatorHydra_Add_Re_Toggle", function(mex, node, ...)
	local params = {
		name = "hydra_re_toggle",
		text_id = "menu_hydra_re_toggle",
		callback = "_update_mutator_value",
		update_callback = callback(mex, mex, "_toggle_hydra_re")
	}
	local data_node = {
	...
	new_item:set_value(mex:use_toggle_hydra_re() and "on" or "off")
...
function MutatorHydra:name()
	...
	if self:_mutate_name("max_unit_depth") then
		...
		local _is_re = self:use_toggle_hydra_re() and "YES" or "NO"
		return string.format("%s - %s , Reverse: %s", name, managers.localization:text("mutator_hydra_split_num", macros), _is_re)
	else
		return name
	end
end
